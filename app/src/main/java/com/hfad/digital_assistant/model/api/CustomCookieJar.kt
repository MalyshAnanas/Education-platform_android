...
//«Менеджер сессий» (или CookieJar). Отвечает за постоянную память приложения.
...
class CustomCookieJar(context: Context) : CookieJar {
    ...
    init {
        try {
            prefs.all.forEach { (key, value) ->
                ...
                val cookies = stringValue.split(";")
                    .mapNotNull { Cookie.parse("https://$key/".toHttpUrl(), it) }
                    .toMutableList()
                cookieStore[key] = cookies
            }
        } catch (e: Exception) {
            Log.e("CustomCookieJar", "Error loading cookies", e)
        }
    }
    ...
    override fun saveFromResponse(url: HttpUrl, cookies: List<Cookie>) {
        ...
        prefs.edit()
            .putString(
                domain,
                existingCookies.joinToString(";") { it.toString() }
            )
            .apply()
    }
